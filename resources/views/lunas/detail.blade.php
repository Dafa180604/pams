...
@section('content')
  <div class="p-4">
    ...
    <div class="bg-white rounded-lg shadow-md">
      ...
      <div class="flex flex-col md:flex-row justify-between">
        ...
        <p class="mt-2 mb-1 font-bold">Nama :
          {{ optional($dataTransaksi->pemakaian->users()->withTrashed()->first())->nama ?? 'Pengguna dihapus' }}</p>
        <p class="mb-3">No Handphone :
          {{ optional($dataTransaksi->pemakaian->users()->withTrashed()->first())->no_hp ?? 'Tidak tersedia' }}</p>
        ...
        <h4 class="font-normal">Nama Petugas/Pencatat : {{ $dataTransaksi->pemakaian->petugas }}</h4>
    ...
    @foreach($kategoriList as $kategori)
        <tr class="border-b">
            <th class="py-2 text-left">{{ $kategori['volume'] }} m³ × Rp {{ number_format($kategori['tarif'], 0, ',', '.') }}</th>
        ...
        <div class="w-full md:w-1/2 md:ml-auto">
          ...
          <table class="w-full table-auto">
            ...
            <tr class="border-b">
              ...
              {{ number_format($dataTransaksi->rp_denda, 0, ',', '.') }}</td>
            </tr>
            ...
            <tr class="border-b">
              ...
              <td class="py-2 px-4 font-bold text-right">Rp
              {{ number_format($dataTransaksi->uang_bayar, 0, ',', '.') }}</td>
            </tr>
            <tr class="border-b">
              <td class="py-2 px-4 font-bold">Kembalian</td>
              <td class="py-2 px-4 font-bold text-right">Rp
              {{ number_format($dataTransaksi->kembalian, 0, ',', '.') }}</td>
            </tr>
            </tbody>
          </table>
          </div>
